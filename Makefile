...
run_mcts_nobuild:
	...
	@docker run -it --ipc=host --gpus all BirdsEye -c /BirdsEye/configs/mcts.yaml 
	@echo
run_dqn: build run_dqn_nobuild
run_dqn_nobuild:
	@echo
	@echo "Running evaluation script with DQN model"
	@docker run -it --ipc=host --gpus all BirdsEye -c /BirdsEye/configs/dqn.yaml 
	@echo
run_batch: build run_batch_nobuild
run_batch_nobuild:
	@echo
	@echo "Running batch evaluation script"
	@docker run -it --ipc=host --gpus all BirdsEye -b
	@echo