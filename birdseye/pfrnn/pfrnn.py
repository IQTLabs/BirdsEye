...
def parse_args(arg_string=None):
    ...
    parser.add_argument('--act_size', type=int, default=6,
    ...
    parser.add_argument('--obs_num', type=int, default=1,
                        help='observation num')
    parser.add_argument('--h_weight', type=float,
                        default=0.1, help='weight for heading loss')
    parser.add_argument('--l2_weight', type=float,
                        default=1.0, help='weight for l2 loss')
    parser.add_argument('--l1_weight', type=float,
                        default=0.0, help='weight for l1 loss')
    parser.add_argument('--elbo_weight', type=float,
                        default=1.0, help='weight for ELBO loss')

    parser.add_argument('--logs_num', type=int, default=0,
...
class pfrnn:
    ...
    def update(self, observation, absolute_pos, action_index):
        ...
        env_map = torch.tensor(np.zeros((self.args.map_size, self.args.map_size))).view(
            1, 1, self.args.map_size, self.args.map_size).float()

        obs = torch.tensor(observation).view(1, 1, -1).float()

        x, y = pol2cart(absolute_pos[0], absolute_pos[1])
        pos = torch.tensor([x, y, np.radians(absolute_pos[2])]).view(
            1, 1, -1).float()
