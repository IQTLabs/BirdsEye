...
class GamutRFSensor(birdseye.sensor.SingleRSSI):
    ...
    def real_observation(self):
        if (self.data.get('rssi', None)) is None or (self.data['rssi'] < self.threshold):
            return None
...
class SigScan:
    ...
    def main(self):
        ...
        while True:
            ...
            action_start = timer()
            self.data['action_proposal'] = planner.proposal(
            ...
            logging.debug('action selection = {:.4f} s'.format(
            ...
            logging.debug('particle save = {:.4f} s'.format(
