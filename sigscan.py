...
def data_handler(message_data):
    global data
    data['previous_position'] = data.get('position', None) if not data.get(
        'needs_processing', True) else data.get('previous_position', None)
    data['previous_bearing'] = data.get('bearing', None) if not data.get(
        'needs_processing', True) else data.get('previous_bearing', None)
    ...
    data['bearing'] = -float(message_data.get('bearing', None)) + \
        90 if is_float(message_data.get('bearing', None)) else data['course']
    data['distance'] = get_distance(
        data['previous_position'], data['position'])
    delta_bearing = (data['bearing'] - data['previous_bearing']
                     ) if data['bearing'] and data['previous_bearing'] else None
    data['action_taken'] = (delta_bearing, data['distance']
                            ) if delta_bearing and data['distance'] else (0, 0)

    data['drone_position'] = message_data.get('drone_position', None)
    if data['drone_position']:
        data['drone_position'] = [data['drone_position']
                                  [1], data['drone_position'][0]]  # swap lon,lat
...
def main(config=None, debug=False):
    ...
    while True:
        ...
        action_start = timer()
        data['action_proposal'] = planner.proposal(
            belief) if planner else [None, None]
        action_end = timer()
        ...
        plot_start = timer()
        results.live_plot(env=env, time_step=time_step,
                          fig=fig, ax=ax, data=data)
        plot_end = timer()

        particle_save_start = timer()
        np.save('{}/{}_particles.npy'.format(results.logdir,
                data['utc_time']), env.pf.particles)
