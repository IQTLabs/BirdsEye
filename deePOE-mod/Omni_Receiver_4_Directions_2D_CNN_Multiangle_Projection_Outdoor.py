...
W = np.matrix(
    [
        [np.cos(1 * (np.pi / 8)), np.sin(1 * (np.pi / 8))],
...
xdata_loc1 = np.concatenate(
    [
        iqdata_loc1["raw-iq"].real.reshape(number_of_data_to_read, 1),
        iqdata_loc1["raw-iq"].imag.reshape(number_of_data_to_read, 1),
    ],
    axis=1,
)
...
xdata_loc2 = np.concatenate(
    [
        iqdata_loc2["raw-iq"].real.reshape(number_of_data_to_read, 1),
        iqdata_loc2["raw-iq"].imag.reshape(number_of_data_to_read, 1),
    ],
    axis=1,
)
...
xdata_loc3 = np.concatenate(
    [
        iqdata_loc3["raw-iq"].real.reshape(number_of_data_to_read, 1),
        iqdata_loc3["raw-iq"].imag.reshape(number_of_data_to_read, 1),
    ],
    axis=1,
)
...
xdata_loc4 = np.concatenate(
    [
        iqdata_loc4["raw-iq"].real.reshape(number_of_data_to_read, 1),
        iqdata_loc4["raw-iq"].imag.reshape(number_of_data_to_read, 1),
    ],
    axis=1,
)
...
xdata_loc1 = xdata_loc1.T.reshape(
    no_of_data_loc1 // (sample_size), sample_size * 8
)  # CHNAGED FROM 2
...
xydata = np.concatenate(
    [xdata.reshape(xdata.shape[0], xdata.shape[1]), ydata.reshape(ydata.shape[0], 1)],
    axis=1,
)
...
ytrain_one_hot = np.reshape(
    ytrain_one_hot, (ytrain_one_hot.shape[0], num_classes)
)  # Used in training
ytest_one_hot = np.reshape(
    ytest_one_hot, (ytest_one_hot.shape[0], num_classes)
)  # Used in training
...
model.add(
    Conv2D(64, (2, 2), input_shape=(8, sample_size, 1), activation="relu")
)  # CHANGE # Stride (1, 1)
...
)  # Multiclass classification with rms adam optimizer # CHANGE
...
loss, accuracy, f1_score, precision, recall = model.evaluate(
    xtest, ytest_one_hot, batch_size=batch_size
)  # CHANGE
print("\nTest Loss: %s: %.2f%%" % (model.metrics_names[0], loss * 100))  # CHANGE
print(
    "\nTest Accuracy: %s: %.2f%%" % (model.metrics_names[1], accuracy * 100)
)  # CHANGE
print("\nTest F1 Score: %s: %.2f%%" % (model.metrics_names[2], f1_score))  # CHANGE
print(
    "\nTest Precision: %s: %.2f%%" % (model.metrics_names[3], precision * 100)
)  # CHANGE
...
def plot_confusion_matrix(
    cm, title="Confusion Matrix", cmap=plt.cm.YlGnBu, labels=[], normalize=False
):
    if normalize:
